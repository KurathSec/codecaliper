...
hand-constructed. The recorded output is `results.txt` (tracked) — re-running the
...
the Java grammar would be a category error (it was the source of an earlier wrong
6% / N=360 figure, since corrected).

All three archives are fetched at run time into the **gitignored**
`validation/bw_faithfulness/cache/`. Their URLs, sha256 checksums, internal layout
and per-corpus licence status are recorded in
[`../bw_faithfulness/dataset.toml`](../bw_faithfulness/dataset.toml).
...
- **Scalabrino et al. (2018)** and **Dorn (2012)**: **no permission has been sought
  ...
  This project only *uses* them — fetches them at run time, measures them, and
  publishes aggregate/derived results (the parse rates below). It does not
  redistribute them and makes no claim that it may.

No corpus content is tracked in this repository. This directory holds only the
script, its recorded output, and two hand-written Python demo files.
...
Missing archives ⇒ the script SKIPs with a precise reason and exits 0 (absence never
fails a build).
