...
Default: the Buse-Weimer corpus only (`bw2010`) — the one the faithfulness
reproduction and the arbitration need. `--all` (or explicit dataset ids) also
fetches the Scalabrino 2018 and Dorn 2012 corpora, which the cross-corpus parse
...
an empty sha256 prints the observed value to record, instead of failing).
Honest SKIP on unavailability, anchor.py style: absence never fails a build.
"""
