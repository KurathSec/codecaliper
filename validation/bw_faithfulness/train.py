...
INPUTS: derived/features.csv (regenerated by extract.py from the TRACKED pinned
snippets if absent) and the TRACKED pinned ratings
derived/arbitration_inputs/scores.csv. No cache/, no network. A missing tracked
input or a missing dependency is a HARD ERROR (stderr, exit 1), never a SKIP: a
lane that silently declines to reproduce its own headline number is the failure
...
SCORES = pinned.SCORES       # TRACKED pinned per-snippet mean ratings (author grant)
...
def main() -> int:
    pinned.require_inputs()  # tracked pins present, or exit 1
    if not FEATURES.exists() or not META.exists():
        import extract  # sibling module; regenerates both from the tracked pins

        rc = extract.main()
        if rc != 0 or not FEATURES.exists():
            print(f"ERROR: extract.py failed (exit {rc}); cannot train.", file=sys.stderr)
            return 1
    try:
        import numpy as np
