...
`validation/bw_faithfulness/dataset.toml` and the cross-corpus lane `validation/breadth/`),
plus the external-oracle differential lane (`tests/differential/` vs
...
`constraints/docs.txt`). W8 release DONE: v0.1.0 tagged 2026-07-11 through `release.yml`
(gate + build in parallel → PyPI trusted publishing → GitHub Release → Zenodo webhook);
PyPI serves `codecaliper 0.1.0`, the GitHub Release is live, and the Zenodo concept DOI
10.5281/zenodo.21312527 resolves (`RELEASING.md` keeps the procedure; Pages enablement done
2026-07-10). Paper sources in `paper/` — LOCAL ONLY, gitignored: submission material stays
out of the public repo until publication (numbers regenerate from derived/, never edited);
the breadth pipeline that backs the cross-corpus numbers is TRACKED in `validation/breadth/`.
Next (ARCHITECTURE.md §16, W8): acmart conversion + MSR Data & Tool Showcase submission
(single-anonymous, 4+1 pages, expected ~Nov 2026 deadline — the CFP has NO screencast or video
requirement; that is an ICPC Tool-Demo convention, not this venue's).
